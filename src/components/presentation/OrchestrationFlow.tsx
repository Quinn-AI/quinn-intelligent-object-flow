...
import { orchestrationSteps, type OrchestrationPhaseId } from "@/lib/presentation-data";
    ...
    <Card className="relative overflow-hidden border-border/70 bg-card/70 backdrop-blur-sm">
      ...
      <CardHeader className="relative z-10 pb-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <CardTitle className="text-lg sm:text-xl">CRO Agent control plane</CardTitle>
            <CardDescription className="text-xs sm:text-sm">
              A continuous read → rank → compose loop. The active step glows as the presentation cycles.
            </CardDescription>
          </div>
      ...
      <CardContent className="relative z-10 space-y-0 px-2 pb-4 sm:px-4">
        <div className="relative">
          <div
            aria-hidden
            className="absolute left-[15px] top-3 bottom-3 w-px bg-gradient-to-b from-border via-sidebar-primary/40 to-border sm:left-[17px]"
          />
            ...
            {orchestrationSteps.map((step) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={cn(
                      "relative overflow-hidden rounded-xl border border-border/70 bg-background/50 p-3 ring-1 ring-transparent transition-all duration-500",
                      active &&
                  ...
                  >
                    ...
                    {active && step.id === "execute" ? (
                      <PulseRing active className="rounded-xl" />
                    ) : null}
                  </div>
                </motion.li>
              );
